...
export function PageLoader() {
  ...
  useEffect(() => {
    if (pathRef.current) {
      setPathLen(Math.ceil(pathRef.current.getTotalLength()));
    }
  }, []);

  useEffect(() => {
    const timer = setTimeout(() => setGone(true), TOTAL + 100);
    return () => clearTimeout(timer);
  }, []);

  if (gone) return null;

  const drawStart = BLANK_DUR;
  const fillStart = drawStart + DRAW_DUR;
  const wordStart = fillStart + FILL_DUR;
  const exitStart = wordStart + WORD_DUR + HOLD_DUR;
  ...
  return (
    <div
      ...
      style={{
        ...
        animation: `loaderExit ${EXIT_DUR}ms ease-in ${exitStart}ms forwards`,
        pointerEvents: "none",
      }}
    >
      ...
      <div className="flex flex-col items-center">
            ...
            style={
              {
                ...
                animation: [
                  `loaderDrawPath ${DRAW_DUR}ms ease-in-out ${drawStart}ms forwards`,
                  `loaderFillIn ${FILL_DUR}ms ease-out ${fillStart}ms forwards`,
                ].join(", "),
        ...
        <svg
          ...
          style={{
            ...
            marginTop: "12px",
            opacity: 0,
            animation: `loaderWordFade ${WORD_DUR}ms ease-out ${wordStart}ms forwards`,
          }}
        >
