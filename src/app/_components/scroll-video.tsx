...
const STATS_LABELS = [
  { label: "Oxygen levels", suffix: "%", decimals: 0 },
  { label: "Chemical pollutants (1)", suffix: "", decimals: 1 },
  { label: "Humidity (2)", suffix: "%", decimals: 0 },
  { label: "Particle pollution (3)", suffix: "", decimals: 0 },
  { label: "Pollen (4)", suffix: "", decimals: 0 },
] as const;
...
export function ScrollVideo() {
  ...
  // Mount animation: count up to outdoor values
  useEffect(() => {
    const animate = (timestamp: number) => {
      ...
      setCounterValues(ENV_OUTDOOR.map((v) => eased * v));
  ...
  // Update stat values based on scroll progress after mount animation
  useEffect(() => {
    if (mountAnimDone.current) {
      setCounterValues(lerpEnvValues(progress));
    }
  }, [progress]);
  ...
  const getGrain = useCallback((w: number, h: number) => {
    ...
    const cached = grainRef.current;
    if (cached && cached.width === gw && cached.height === gh) {
      return cached;
    }
          ...
          >
            {STATS_LABELS.map((item, i) => (
              <React.Fragment key={item.label}>
                ...
                >
                  {(counterValues[i] ?? 0).toFixed(item.decimals)}
                  {item.suffix}
                </span>
              </React.Fragment>
            ))}
